...
export default function ReportPrint() {
  ...
  if (!r || !d) return <div className="p-10 text-center text-ink-2">Report not found.</div>

  const s = r.snapshot
  const stale = s.area_km2 !== d.area_km2 || s.status !== d.status || s.top_vessel !== d.top_vessel || s.ranked_count !== d.ranking.length
  ...
  return (
    <div className="min-h-dvh bg-surface-2 print:bg-white">
      <div className="no-print sticky top-0 z-10 flex items-center justify-between border-b border-line bg-surface px-6 py-3">
        <Link to={`/app/incidents/${d.id}`} className="inline-flex items-center gap-1.5 text-sm text-ink-2 hover:text-ink"><ArrowLeft className="size-4" />Back to investigation</Link>
      ...
      <main className="mx-auto max-w-[210mm] bg-white p-[14mm] text-ink shadow-sm print:max-w-none print:p-0 print:shadow-none">
        ...
        <header className="mb-6 flex items-start justify-between border-b-2 border-ink pb-4">
          ...
          <div className="text-right text-xs text-ink-2">
            <div className="font-mono font-semibold text-ink">{d.code} · rev. {r.revision}</div>
        ...
        <section className="mb-6 grid grid-cols-2 gap-6">
          ...
          <div>
            ...
            <dl className="space-y-1 text-xs">
              <Row k="Tile SHA-256"><span className="break-all font-mono">{d.hashes.tile_sha256 ?? '—'}</span></Row>
              <Row k="Mask SHA-256"><span className="break-all font-mono">{d.hashes.mask_sha256 ?? '—'}</span></Row>
              <Row k="Candidates considered">{d.candidates_considered ?? '—'}</Row>
              <Row k="Vessels ranked">{d.ranking.length}</Row>
            </dl>
          </div>
        </section>
